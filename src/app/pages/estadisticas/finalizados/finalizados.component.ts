...
export class FinalizadosComponent implements OnInit, AfterViewInit {
  ...
  finalizados: Consulta[] = [];
  finalizadosAno: Consulta[] = [];
  ...
  dtElement: DataTableDirective;
	...
	public finalizadosChartLabels:string[] = [];
	public finalizadosChartData:any[] = [];
	public finalizadosChartType:string = 'line';
  ...
  cargarDatosAno( a = this.anio ) {
  	...
  	this._consultaService.cargarAno( a, 'turnos' )
			.subscribe( (resp: any) => {
				...
				let finalizadosAno:any = resp.consultas.filter( c => !c.deleted && c.status === 'FINALIZADO');
				let n_months = Array(12).fill(0).map((e,i)=>i+1);
				//let r_data = finalizadosAno.map( (c: any) => c.date_t.split(('-'))[1]), // fill it with array with your data
				
				let r_data = finalizadosAno.map( (c: any) => {
					//console.log(c._id);
		    ...
		    this.finalizadosAno = r_the_months.map(x => Object.assign(x, r_rarr.find(y => y.month == x.month)));
  ...
  cargarDatosRango( desde = this.from, hasta = this.to ) {
		...
		this.finalizadosChartLabels = [];
		this.finalizadosChartData = [];
		...
		this._consultaService.cargarRango( desde, hasta, 'turnos' )
			.subscribe( (resp: any) => {
				...
				this.finalizados = resp.consultas.filter( c => !c.deleted && c.status === 'FINALIZADO');
				console.log('this.finalizados', this.finalizados);
	...
	aplicarFiltroGrafico() {
		...
		//----------  prepare chart  ----------				
		let reservas = this.finalizados.filter(c => c.status === 'CONSULTA'),
				reservados = this.finalizados.filter(c => c.status === 'RESERVADO'),
				finalizados = this.finalizados.filter(c => c.status === 'FINALIZADO'),
				cancelados = this.finalizados.filter(c => c.status === 'CANCELADO'),
				results = [];
    ...
    results = [
		...
		this.finalizadosChartLabels = this.dias_label;
		this.finalizadosChartData = results;

		/*----------  get medios de contacto  ----------*/
		let medios = this.finalizados.filter(c => c.medio_c !== undefined && c.medio_c !== null).map(c => c.medio_c);
		...
		let place = this.finalizados.filter(c => c.place_t !== undefined && c.place_t !== null).map((c:any) => c.place_t.name);
	...
	aplicarFiltroLista() {
		...
		//this.finalizadosChartLabels = [];
		//this.finalizadosChartData = [];
		let the_finalizados = this.finalizados;

		this.c_filtro = the_finalizados.filter((c: any) => {
    ...
    //results = the_dias.map(x => Object.assign(x, rarr.find(y => y.date == x.date)));
		...
		//this.finalizadosChartLabels = this.dias_label;
		//this.finalizadosChartData = [ { data: results.map( r => r.count), label: this.tipo } ];
