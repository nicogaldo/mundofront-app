...
export class ContactosComponent implements OnInit, AfterViewInit {
  ...
  dtElement: DataTableDirective;
	...
	public consultasChartData:any[] = [];
	public consultasChartType:string = 'line';
  ...
  ngOnInit() {
      ...
      scales: {
 			...
 			buttons: [
        {
	        ...
	        title: 'Estadisticas de ' + moment.utc(this.selectMes).format('MMMM')
        },
        {
        	extend: 'print',
  ...
  ngAfterViewInit(): void {
	...
	getDaysArrayByRango( from, to ) {
	  ...
	  /*let daysInMonth = moment.utc([this.selectAnio, this.selectMes]).daysInMonth();
	  let arrDays = [];
	  let arrDays_label = [];

	  while(daysInMonth) {
	    let current = moment.utc([this.selectAnio, this.selectMes]).date(daysInMonth).format('YYYY-MM-DD');
	    let current_label = moment.utc([this.selectAnio, this.selectMes]).date(daysInMonth).format('ddd DD');
	    arrDays.push(current);
	    arrDays_label.push(current_label);
	    daysInMonth--;
	  }

	  this.dias_label;
	  this.dias = arrDays.reverse();
	  this.dias_label = arrDays_label.reverse();
	  //return arrDays;*/
  ...
  cargarDatosAno( a = this.anio ) {
  	...
  	this._consultaService.cargarAno( a )
			.subscribe( (resp: any) => {

				let consultasAno:any = resp.consultas.filter( c => !c.deleted);
				//console.log('consultasAno');
				//console.log(consultasAno);

				let n_months = Array(12).fill(0).map((e,i)=>i+1);
				let r_data = consultasAno.map( (c: any) => c.createdAt.split(('-'))[1]), // fill it with array with your data
				r_rarr = [], r_the_months = [];
		    ...
		    this.consultasAno = r_the_months.map(x => Object.assign(x, r_rarr.find(y => y.month == x.month)));
			});
  }
  ...
  cargarDatosRango( desde = this.from, hasta = this.to ) {
		...
		this._consultaService.cargarRango( desde, hasta )
			.subscribe( (resp: any) => {
	...
	aplicarFiltroGrafico() {
		...
		//----------  prepare chart  ----------				
		let consultas = this.consultas.filter(c => c.status === 'CONSULTA'),
				reservados = this.consultas.filter(c => c.status === 'RESERVADO'),
				finalizados = this.consultas.filter(c => c.status === 'FINALIZADO'),
				cancelados = this.consultas.filter(c => c.status === 'CANCELADO'),
				results = [];

		//----------- consultas -----------
		let c_date = consultas.map( (c: any) => c.date_filter), // fill it with array with your data
    ...
    results = [
		...
		this.consultasChartLabels = this.dias_label;
		this.consultasChartData = results;

		/*----------  get medios de contacto  ----------*/
		let medios = this.consultas.filter(c => c.medio_c !== undefined && c.medio_c !== null).map(c => c.medio_c);
		...
		let place = this.consultas.filter(c => c.place_c !== undefined && c.place_c !== null).map((c:any) => c.place_c.name);
	...
	aplicarFiltroLista() {
		...
		//this.consultasChartLabels = [];
		//this.consultasChartData = [];
		let the_consultas = this.consultas;

		this.c_filtro = the_consultas.filter((c: any) => {

			c.date_filter = moment.utc(c.createdAt).format('YYYY-MM-DD');

			// Selecciono 3 filtros
			if (this.tipo && this.place && this.contact) {
				return c.status === this.tipo && (c.place_c && c.place_c._id === this.place) && c.medio_c === this.contact;

			// Selecciono Tipo y Lugar
			} else if (this.tipo && this.place) {
				return c.status === this.tipo && (c.place_c && c.place_c._id === this.place);
			...
			// Selecciono Contacto y Lugar
				...
				return (c.place_c && c.place_c._id === this.place) && c.medio_c === this.contact;
			...
			// Selecciono Lugar
				...
				return c.place_c && c.place_c._id === this.place;
    ...
    //results = the_dias.map(x => Object.assign(x, rarr.find(y => y.date == x.date)));
		...
		//this.consultasChartLabels = this.dias_label;
		//this.consultasChartData = [ { data: results.map( r => r.count), label: this.tipo } ];
